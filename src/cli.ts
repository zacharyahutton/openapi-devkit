...
import { generateClient, generateInterfaces, formatEndpointList } from "./generate.js";
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (key === "--list-endpoints") args.listEndpoints = true;
    else if (key.startsWith("--") && argv[i + 1]) args[key.slice(2)] = argv[++i];
  }
  return args;
}
...
const listEndpoints = Boolean(args.listEndpoints);

const spec = parseSpec(readFileSync(resolve(input), "utf8"));

if (listEndpoints) {
  console.log(formatEndpointList(spec));
  process.exit(0);
}
